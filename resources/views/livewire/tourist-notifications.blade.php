<div
    class="relative"
    x-data="{ open: false }"
    x-init="if (window.Echo) { window.Echo.private('App.Models.User.{{ (int) auth()->id() }}').notification(() => { $wire.$refresh(); }); }"
        ...
        <div class="max-h-80 overflow-y-auto px-2 py-2">
            @forelse($this->notifications as $notification)
                ...
                <button
                    ...
                    class="mb-1 flex w-full items-start gap-3 rounded-xl px-3 py-2 text-left transition {{ $isUnread ? 'bg-[#fff2df] hover:bg-[#fcead0]' : 'hover:bg-[#f8efdf]' }}"
                >
                    <span class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full {{ $isUnread ? 'bg-[#f7e0bf] text-[#7d583b]' : 'bg-[#f4ead8] text-[#8a7561]' }}">
                        <i class="fa-solid {{ $icon }} text-xl"></i>
                    </span>
                    <span class="min-w-0 flex-1">
                        <span class="flex items-center gap-2">
                            <span class="truncate text-sm font-semibold text-[#5a3c2a]">{{ $title }}</span>
                            @if($isUnread)
                                <span class="h-2 w-2 rounded-full bg-[#c0604a]"></span>
                            @endif
                        </span>
                        <span class="line-clamp-2 text-xs text-[#7a6654]">{{ $message }}</span>
                        <span class="mt-1 block text-[11px] text-[#9a836d]">{{ $notification->created_at?->diffForHumans() }}</span>
                    </span>
                </button>
            @empty
