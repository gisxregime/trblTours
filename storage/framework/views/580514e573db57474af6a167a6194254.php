...
<article class="relative flex flex-col overflow-hidden rounded-[18px] border border-[#e7dcc8] bg-[#f8f5ef] shadow-[0_8px_22px_rgba(52,41,26,0.08)] transition hover:shadow-[0_12px_28px_rgba(52,41,26,0.12)]">
    <a href="<?php echo e(route('tours.show', ['tour' => $tour, 'from' => $sourceContext])); ?>" data-tour-card-link="tour-<?php echo e($tour->id); ?>" class="absolute inset-0 z-10" aria-label="View details for <?php echo e($finalTitle); ?>"></a>

    <div class="relative h-[176px] w-full">
        <img src="<?php echo e($finalImageUrl); ?>" class="h-full w-full object-cover" alt="<?php echo e($finalTitle); ?>">

        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->guard()->check()): ?>
            <form method="POST" action="<?php echo e(route('dashboard.likes.toggle', $tour)); ?>" class="absolute right-3 top-3 z-20" data-like-form="true" data-like-ajax="<?php echo e($enableAjax ? 'true' : 'false'); ?>" data-liked="<?php echo e($isLikedByCurrentUser ? 'true' : 'false'); ?>">
                <?php echo csrf_field(); ?>
                <button type="submit" data-like-button="true" class="inline-flex h-8 w-8 items-center justify-center rounded-full border border-[#dfd7c8] bg-white/95 <?php echo e($isLikedByCurrentUser ? 'text-[#b8505b]' : 'text-[#8b7f6e]'); ?> shadow-sm transition hover:text-[#6d5d4c] disabled:cursor-not-allowed disabled:opacity-70" aria-label="<?php echo e($heartActionLabel); ?>" aria-pressed="<?php echo e($isLikedByCurrentUser ? 'true' : 'false'); ?>">
                    <i data-like-icon="true" class="<?php echo e($isLikedByCurrentUser ? 'fa-solid' : 'fa-regular'); ?> fa-heart"></i>
                </button>
            </form>
        <?php else: ?>
            <a href="<?php echo e(route('login')); ?>" class="absolute right-3 top-3 z-20 inline-flex h-8 w-8 items-center justify-center rounded-full border border-[#dfd7c8] bg-white/95 text-[#8b7f6e] shadow-sm transition hover:text-[#6d5d4c]" aria-label="Login to like this tour">
        ...
        <div class="absolute -bottom-4 left-3 right-3 flex items-center gap-1.5 overflow-hidden rounded-full border border-[#e3ddd0] bg-white/95 px-2 py-1 shadow-sm">
            <img src="<?php echo e($finalGuideAvatar); ?>" class="h-5 w-5 rounded-full object-cover border border-[#ede1cd]" alt="<?php echo e($finalGuideName); ?>">
            <span class="truncate text-[10px] font-medium text-[#8a7f70]">Your Guide: <?php echo e($finalGuideName); ?></span>
        </div>
    </div>

    <div class="flex flex-grow flex-col px-4 pb-4 pt-7">
        <h3 class="mb-1 text-[21px] font-semibold leading-tight text-[#5f5447]"><?php echo e($finalTitle); ?></h3>

        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($finalRegion !== ''): ?>
            <p class="mb-1 text-[12px] font-medium text-[#7f7262]"><?php echo e($finalRegion); ?></p>
        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>

        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($finalSummary !== ''): ?>
            <p class="mb-2 line-clamp-2 text-[12px] text-[#887a69]"><?php echo e($finalSummary); ?></p>
        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
        ...
        <div class="mt-3 flex items-center justify-between border-t border-[#e5dcc9] pt-3">
            <span class="text-[11px] font-semibold text-[#877a6a]">Total <span class="text-[16px] font-bold text-[#5d5142]">₱ <?php echo e(number_format((float) $finalPrice, 2)); ?></span></span>
        </div>
    </div>
</article>
...
<?php endif; ?>
<?php /**PATH /home/mistah-regime/tribaltours/resources/views/components/explore-tour-card.blade.php ENDPATH**/ ?>