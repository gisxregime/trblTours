...
<body class="min-h-screen bg-[#f5efe2] font-inter text-[#3e2a1f]"
    x-data="myPostsPage({
        currentUserId: <?php echo \Illuminate\Support\Js::from((int) auth()->id())->toHtml() ?>,
        posts: <?php echo \Illuminate\Support\Js::from($bootstrapPosts)->toHtml() ?>,
        demoPosts: <?php echo \Illuminate\Support\Js::from($demoPosts)->toHtml() ?>,
        stats: <?php echo \Illuminate\Support\Js::from($stats)->toHtml() ?>,
        conversations: <?php echo \Illuminate\Support\Js::from($bootstrapConversations)->toHtml() ?>,
        showUrlTemplate: <?php echo \Illuminate\Support\Js::from($showUrlTemplate)->toHtml() ?>,
        storeUrlTemplate: <?php echo \Illuminate\Support\Js::from($storeUrlTemplate)->toHtml() ?>,
        readUrlTemplate: <?php echo \Illuminate\Support\Js::from($readUrlTemplate)->toHtml() ?>,
        updateUrlTemplate: <?php echo \Illuminate\Support\Js::from(route('dashboard.my-posts.update', ['touristRequest' => '__post__']))->toHtml() ?>,
        deleteUrlTemplate: <?php echo \Illuminate\Support\Js::from(route('dashboard.my-posts.delete', ['touristRequest' => '__post__']))->toHtml() ?>,
        completeUrlTemplate: <?php echo \Illuminate\Support\Js::from(route('dashboard.my-posts.complete', ['touristRequest' => '__post__']))->toHtml() ?>,
        commentsUrlTemplate: <?php echo \Illuminate\Support\Js::from(route('dashboard.my-posts.comments', ['touristRequest' => '__post__']))->toHtml() ?>,
        storeCommentUrlTemplate: <?php echo \Illuminate\Support\Js::from(route('dashboard.my-posts.comments.store', ['touristRequest' => '__post__']))->toHtml() ?>,
        selectGuideUrlTemplate: <?php echo \Illuminate\Support\Js::from(route('dashboard.my-posts.comments.select-guide', ['touristRequest' => '__post__', 'comment' => '__comment__']))->toHtml() ?>,
        reportCommentUrlTemplate: <?php echo \Illuminate\Support\Js::from(route('dashboard.my-posts.comments.report', ['touristRequest' => '__post__', 'comment' => '__comment__']))->toHtml() ?>,
        requestStoreUrl: <?php echo \Illuminate\Support\Js::from(route('requests.store'))->toHtml() ?>,
        ...
        <header :class="scrolled ? 'nav-shell scrolled' : 'nav-shell'" class="fixed inset-x-0 top-0 z-50 border-b border-[#ddd2bd]">
            <div class="mx-auto flex max-w-[1260px] items-center justify-between px-6 py-4">
                ...
                <nav class="hidden items-center gap-10 md:flex">
                    <a href="<?php echo e(route('dashboard.tourist')); ?>" class="top-nav-link">Explore Tours</a>
                    <a href="<?php echo e(route('dashboard.my-posts')); ?>" class="top-nav-link active">My Posts</a>
                </nav>
...
$__key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-1020074827-0', $__key);
...
?>
                    ...
                    <div x-cloak x-show="menuOpen" x-transition class="absolute right-0 top-full mt-2 w-56 overflow-hidden rounded-2xl border border-[#d8c7a9] bg-[#fffaf2] py-2 shadow-2xl">
                        ...
                        <a href="<?php echo e(route('dashboard.my-bookings')); ?>" class="flex items-center gap-2 px-4 py-2.5 text-sm text-[#5a3c2a] transition hover:bg-[#f7ecd7]">
                            <i class="fa-solid fa-calendar-check w-4 text-center"></i>
                            <span>Bookings</span>
                        </a>
                        <a href="<?php echo e(route('dashboard.likes')); ?>" class="flex items-center gap-2 px-4 py-2.5 text-sm text-[#5a3c2a] transition hover:bg-[#f7ecd7]">
    ...
    <script>
        (() => {
            ...
            form.addEventListener('submit', async (event) => {
                ...
                try {
                    const response = await fetch(<?php echo \Illuminate\Support\Js::from(route('requests.store'))->toHtml() ?>, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': <?php echo \Illuminate\Support\Js::from(csrf_token())->toHtml() ?>,
                            Accept: 'application/json',
                        },
                        body: formData,
                    });
...
<?php /**PATH /home/mistah-regime/tribaltours/resources/views/dashboards/my-posts.blade.php ENDPATH**/ ?>